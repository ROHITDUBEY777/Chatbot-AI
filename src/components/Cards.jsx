    ...
    <motion.div
         ref={refcard}
          ...
          transition={{duration:1,ease:"easeOut"}}
           ...
           className={`flex flex-col gap-34   justify-center ${darkmode?"text-white ":"text-black "}   px-4  lg:flex-row md:gap-24   md:px-4 lg:px-24  min-h-screen     `}>
       {cardsobj.map((item)=>(
         <div
         ...
         <ul className=' w-fit h-fit md:h-fit  justify-between items-center text-center  gap- '>
          <li className='mt-[4vh]  md:px-6 lg:mt-[8vh]   mb-11 text-xl'>{item.title}</li>
          <p className={`rounded-lg  text-center py-4 md:px-6 xl:h-[24rem]  px-4 md:py-10  xl:px-12 ${darkmode?"bg-[#27272a] ":"bg-[#e7e5e4]"}   text-base`}>{item.description}</p>
         </ul>
         </div>
       ))}
